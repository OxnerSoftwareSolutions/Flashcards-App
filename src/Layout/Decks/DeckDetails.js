...
function DeckDetails() {
  ...
  const getDeckDetails = useCallback(async () => {
    try {
      const deck = await readDeck(deckId);// pulled from the api utils 
      /*
  ...
  async function deleteHandler(id) {
    if (
      window.confirm("Delete this card?\n\nYou will not be able to recover it.")// pulls up a confirmation dialog with the user to confirm their actions
    ) {
      await deleteCard(id);
      getDeckDetails();
    }
